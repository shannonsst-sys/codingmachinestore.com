...
export default function CartIsland({ floating = false }: Props) {
  ...
  return <div className={`cart-island${floating ? ' cart-island-floating' : ''}`}>
    ...
    {open && <><button className="cart-modal-backdrop" type="button" aria-label="Close cart" onClick={() => setOpen(false)} /><section className="cart-modal" role="dialog" aria-modal="true" aria-label="Your quote list"><div className="cart-modal-header"><div><span className="eyebrow">Your order</span><h2>Quote list <small>{count} items</small></h2></div><button className="cart-modal-close" type="button" onClick={() => setOpen(false)} aria-label="Close cart">×</button></div>{cart.length === 0 ? <div className="cart-modal-empty"><strong>Your cart is empty.</strong><p>Add products to compare quantities and request a quote.</p></div> : <div className="cart-modal-items">{cart.map((line) => <div className="cart-modal-line" key={line.id}><div className="cart-modal-line-copy"><strong>{line.name}</strong><small>${line.price.toFixed(2)} / unit</small></div><div className="cart-modal-line-controls"><div className="quantity-control"><button type="button" onClick={() => updateQuantity(line.id, line.quantity - 1)} aria-label={`Decrease ${line.name}`}>−</button><input type="number" min="1" step="1" value={line.quantity} onChange={(event) => updateQuantity(line.id, Number(event.target.value))} aria-label={`${line.name} quantity`} /><button type="button" onClick={() => updateQuantity(line.id, line.quantity + 1)} aria-label={`Increase ${line.name}`}>＋</button></div><strong className="cart-modal-line-total">${(line.price * line.quantity).toFixed(2)}</strong><button className="cart-modal-remove" type="button" onClick={() => removeLine(line.id)}>Remove</button></div></div>)}</div>}<div className="cart-modal-footer"><div><span>Products total</span><strong>${total.toFixed(2)}</strong></div><p>Shipping is calculated separately after we receive your delivery address.</p><a className="button button-dark" href="/#quote" onClick={() => setOpen(false)}>Request a quote</a></div></section></>}
  </div>;
}
