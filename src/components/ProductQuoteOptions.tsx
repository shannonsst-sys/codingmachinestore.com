...
export default function ProductQuoteOptions({ id, name, options, notesPrompt }: Props) {
  ...
  const add = () => {
    ...
    const lineNotes = isStructuredCustomization ? structuredNotes : notes.trim();
    ...
    else current.push({ id: lineId, name: lineName, price: selectedValue.price, quantity, sku: selectedValue.sku, notes: lineNotes || undefined, quoteOnly: selectedValue.quoteOnly });
  ...
  return <div className="product-options">
    ...
    {isStructuredCustomization ? <div className="product-option-character-quantities"><div className="product-option-character-heading"><span>Character quantities *</span><strong>{structuredTotal} / {characterLimit}</strong></div><div className="character-quantity-grid">{customCharacters.map((character) => <label className="character-quantity" key={character}><span>{character === 'blank' ? 'Blank space' : character}</span><input type="number" min="0" max={characterLimit} step="1" value={customQuantities[character]} onChange={(event) => { const value = Math.max(0, Math.floor(Number(event.target.value) || 0)); setCustomQuantities({ ...customQuantities, [character]: value }); setMessage(''); }} aria-label={`Quantity for ${character === 'blank' ? 'blank space' : character}`} /></label>)}</div><small>Enter the quantity for each character. The total must be exactly {characterLimit} pieces before this custom box can be added.</small></div> : requiresNotes && <label className="product-option-notes"><span>Customization details *</span><textarea value={notes} onChange={(event) => { setNotes(event.target.value); setMessage(''); }} rows={4} maxLength={1200} required placeholder={notesPrompt ?? 'List the letters, numbers and symbols with quantities, for example: A×5, 0×10, /×2, blank×3.'} /></label>}
