...
export default function QuoteForm() {
  ...
  const selectedProducts = cart.length
    ? cart.map((line) => `${line.name} × ${line.quantity} — $${(Number(line.price || 0) * Number(line.quantity || 0)).toFixed(2)}`).join('\n')
  ...
  return <form className="quote-form-modern" action="https://formspree.io/f/xppalleq" method="POST" onSubmit={validateContact}>
    ...
    <div className="quote-form-summary"><span>{cart.length ? `${cart.length} product line${cart.length === 1 ? '' : 's'} in your quote` : 'No products in your quote yet'}</span><strong>${total.toFixed(2)}</strong></div>
