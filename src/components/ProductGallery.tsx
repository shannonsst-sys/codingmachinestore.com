...
export default function ProductGallery({ name, images }: Props) {
  ...
  return <div className="gallery">
    ...
    <div className="gallery-thumbs" role="list">{images.map((image, index) => <button type="button" className={index === active ? 'is-active' : ''} onClick={() => setActive(index)} key={image}><img src={image} alt={`${name} view ${index + 1}`} /></button>)}</div>
